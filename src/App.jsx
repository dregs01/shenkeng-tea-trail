...
export default class App extends React.Component {
          ...
          <div style={s("position:absolute;inset:0;background:#efe6d3;display:flex;flex-direction:column;")}>
            <div style={s("display:flex;align-items:center;gap:12px;padding:14px 16px;background:#f4ecd9;border-bottom:1px solid rgba(59,52,42,.12);z-index:5;")}>
              ...
              <div style={s("flex:1;")}>
                ...
                <div style={s("font-family:'LXGW WenKai TC',cursive;font-size:22px;color:#3b342a;line-height:1;")}>{sceneTitle}</div>
                <div style={s("font-size:11.5px;color:#9a7b4f;")}>{sceneSubtitle}</div>
              </div>
            ...
            <div style={s("flex:1;display:flex;flex-direction:column;position:relative;overflow-y:auto;")}>
              <div style={s("position:relative;width:100%;aspect-ratio:1000/476;flex:0 0 auto;")}>
                <img src={sceneImg} alt="蔡家古厝手繪插畫" style={s("position:absolute;inset:0;width:100%;height:100%;object-fit:cover;")} />
                ...
                {positions.map((pos, i) => (
                  <button key={i} onClick={() => this.openQuiz(i)} style={{ ...s("position:absolute;transform:translate(-50%,-50%);width:46px;height:46px;padding:0;border:none;background:transparent;cursor:pointer;z-index:4;"), left: pos.left, top: pos.top }}>
                    ...
                    <span style={sealStyle(i)}>{this.hotspots()[i].char}</span>
                ...
                {/* 奉茶 */}
                  {!isLin && (
                    <button onClick={() => this.openTea()} style={s("position:absolute;left:68%;top:75%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;gap:4px;padding:0;border:none;background:transparent;cursor:pointer;z-index:5;")}>
                      <span style={s("position:relative;width:50px;height:50px;display:flex;align-items:center;justify-content:center;")}>
                        {showHalo && !st.teaDone && <span style={s("position:absolute;left:50%;top:50%;width:64px;height:64px;border-radius:50%;border:2px solid rgba(198,154,69,.55);animation:halo 2.4s ease-out infinite;")}></span>}
  ...
  const A = this.props.accentColor ?? '#b15a3c'
  const totalSolved = solved.filter(Boolean).length + linSolved.filter(Boolean).length
  const totalCards = HOTSPOTS.length + LIN_HOTSPOTS.length

  const renderCard = (h, isSolved) => (
    isSolved ? (
      /* 已解開：完整卡片 */
    ...
    <div style={s("position:absolute;inset:0;background:#efe6d3;display:flex;flex-direction:column;")}>
      ...
      <div style={s("display:flex;align-items:center;gap:12px;padding:14px 16px;background:#f4ecd9;border-bottom:1px solid rgba(59,52,42,.12);")}>
        ...
        <div style={s("flex:1;")}>
          ...
          <div style={s("font-size:11.5px;color:#9a7b4f;")}>已解開 {totalSolved}/{totalCards} 張</div>
        </div>
      </div>
      ...
      <div style={s("flex:1;overflow-y:auto;padding:18px 18px 30px;")}>
        ...
        {/* 蔡家古厝 */}
        <div style={s("font-family:'LXGW WenKai TC',cursive;font-size:19px;color:#9a7b4f;margin:4px 2px 10px;")}>蔡家古厝</div>
        {HOTSPOTS.map((h, i) => (
          <div key={'cai-' + i} style={{ ...s("border-radius:16px;overflow:hidden;margin-bottom:14px;box-shadow:0 2px 10px rgba(59,52,42,.1);"), opacity: solved[i] ? 1 : 0.5 }}>
            {renderCard(h, solved[i])}
          </div>
        ))}

        {/* 林家草厝 */}
        <div style={s("font-family:'LXGW WenKai TC',cursive;font-size:19px;color:#9a7b4f;margin:22px 2px 10px;")}>林家草厝</div>
        {LIN_HOTSPOTS.map((h, i) => (
          <div key={'lin-' + i} style={{ ...s("border-radius:16px;overflow:hidden;margin-bottom:14px;box-shadow:0 2px 10px rgba(59,52,42,.1);"), opacity: linSolved[i] ? 1 : 0.5 }}>
            {renderCard(h, linSolved[i])}
          </div>
        ))}

        {/* 全部解開後的彩蛋文字 */}
        {solved.every(Boolean) && linSolved.every(Boolean) && (
          <div style={s("text-align:center;padding:20px 10px;")}>
            ...
            <p style={s("font-size:13px;color:#9a7b4f;line-height:1.7;margin:0;")}>你已把蔡家古厝與林家草厝的生活智慧都收進口袋，<br/>是真正的茶山知己。</p>
          </div>
        )}
      </div>
    </div>
